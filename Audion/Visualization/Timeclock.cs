...
namespace Audion.Visualization
{
    ...
    public class Timeclock : Control
    {
        ...
        public static readonly DependencyProperty ClockTypeProperty = DependencyProperty.Register("ClockType", typeof(ClockType),
            typeof(Timeclock), new UIPropertyMetadata(ClockType.TimeElapsed, OnClockTypeChanged, OnCoerceClockType));

        private static object OnCoerceClockType(DependencyObject o, object value)
        {
            Timeclock timeclock = o as Timeclock;

            if (timeclock != null)
                return timeclock.OnCoerceClockType((ClockType)value);
            else
                return value;
        }
        ...
        private static void OnClockTypeChanged(DependencyObject o, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (timeclock != null)
                timeclock.OnClockTypeChanged((ClockType)e.OldValue, (ClockType)e.NewValue);
        }
        ...
        protected virtual ClockType OnCoerceClockType(ClockType value)
        {
            return value;
        }
        ...
        protected virtual void OnClockTypeChanged(ClockType oldValue, ClockType newValue)
        {
            CalculateTime();
        }
        ...
        public ClockType ClockType
        {
            get
            {
                return (ClockType)GetValue(ClockTypeProperty);
            }
            set
            {
        ...
        private void SourceCollection_CollectionChanged(object sender, System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
        {
            ...
            if (e.OldItems != null)
            {
                ...
                {
                    (source as ISource).SourcePropertyChangedEvent -= _source_SourcePropertyChangedEvent;
                }
            }
        }
        ...
        private void CalculateTime()
        {
            if (_activeSource != null)
            {
                ...
                {
                    if (ClockType == ClockType.TimeElapsed)
                    {
                        timeText.Text = position.ToString(TimeFormat);
                    }
                    else if (ClockType == ClockType.TimeRemaining && isOutputSource)
                    {
